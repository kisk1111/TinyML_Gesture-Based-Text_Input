...
def main():
    ...
    student_name = "big"
    kd_keras_path = os.path.join(KD_DIR, f"model_kd_{student_name}.keras")

    if not os.path.exists(kd_keras_path):
        print(f"ERROR: {kd_keras_path} not found.")
        return

    print(f"\nLoading KD student '{student_name}' from: {kd_keras_path}")
    try:
        model = tfk.models.load_model(kd_keras_path)
    except Exception:
        import keras
        model = keras.models.load_model(kd_keras_path)
    ...
    print("Float16 Quantisation")
    fp16_path = os.path.join(OUTPUT_DIR, f"model_kd_{student_name}_quant_fp16.tflite")
    ...
    int8_path = os.path.join(OUTPUT_DIR, f"model_kd_{student_name}_quant_int8.tflite")
    int8_size = quantise_int8(export_model, int8_path, make_representative_dataset(X_train))
    int8_acc = evaluate_tflite(int8_path, X_test, y_test)
    print(f"  Size:     {int8_size:.1f} KB")
    print(f"  Accuracy: {int8_acc * 100:.2f}%")

    print("SUMMARY")
    print(f"{'Model':<30} {'Size (KB)':>10} {'Accuracy':>10} {'Size Red.':>10}")
    print(f"{'Baseline (float32)':<30} {baseline_size:>10.1f} {'98.27%':>10} {'-':>10}")
    print(f"{'KD big (float32)':<30} {'?':>10} {'?':>10} {'-':>10}")
    print(f"{'KD big + Float16':<30} {fp16_size:>10.1f} {fp16_acc*100:>9.2f}% {(1 - fp16_size/baseline_size)*100:>9.1f}%")
    print(f"{'KD big + Int8':<30} {int8_size:>10.1f} {int8_acc*100:>9.2f}% {(1 - int8_size/baseline_size)*100:>9.1f}%")
